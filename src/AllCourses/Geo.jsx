...
const Geo=({progress}) => {
  ...
  const [quiz, setQuiz] = useState([]);  // Store quizzes

  useEffect(() => {
    const getLesson = async () => {
      try {
        const res = await axios.get("http://localhost:4001/lesson");
        const proglesson = res.data.filter(lesson => lesson.category === "geo");
  ...
  return (
    <>
      <div className='pt-10 px-4 md:px-10 lg:px-20 dark:text-white dark:bg-slate-900 text-black bg-white '>
        <h1 className='text-left text-xl pt-20 md:text-3xl lg:text-4xl'>Learn Geography</h1>
